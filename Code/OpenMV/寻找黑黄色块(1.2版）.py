...
while(True):
    ...
    if rects:
        for rect in rects:
            ...
            blobs = img.find_blobs([black,yellow],roi = rect.rect(), x_stride=5, y_stride=3, invert=False, area_threshold=200, pixels_threshold=200, merge=False)
            for blob in blobs:
                #img.draw_rectangle(blob.rect(),color=(255,0,0))
                if 80<=blob.cx()<=90:   #设置一个值还是一个范围有待商榷
                    num+=1
    else:
        blobs = img.find_blobs([black,yellow], x_stride=5, y_stride=3, invert=False, area_threshold=200, pixels_threshold=200, merge=False)
        for blob in blobs:
            #img.draw_rectangle(blob.rect(),color=(255,0,0))
            if 80<=blob.cx()<=90:   #设置一个值还是一个范围有待商榷
                num+=1
