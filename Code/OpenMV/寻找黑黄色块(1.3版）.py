...
yellow = (30, 60, -14, 12, 12, 52)#黄色胶带（下午）
num_black = 0
num_yellow = 0
    ...
    if rects:
        for rect in rects:
            ...
            for blob in black_blobs:
                ...
                if 8<=(blob.h()/blob.w())<=15:#后续确定好飞机高度以及与杆的距离之后还要修改长宽比
            ...
            for blob in yellow_blobs:
                img.draw_rectangle(blob.rect(),color=(255,0,0))
                if 1<=(blob.h()/blob.w())<=2:
                    if 80<=blob.cx()<=90:   #设置一个值还是一个范围有待商榷
                        num_yellow+=1
                        key = 200
                        break
    elif (num_black+num_yellow) == 3:
        atcion1 = 100
    elif (num_black+num_yellow) == 6:
        action2 = 100
