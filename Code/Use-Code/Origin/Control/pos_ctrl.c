...
void POS_Ctrl(float dT_s)
{
	//====input calcu====
	//
	exp_velocity[0] = -(Remote.pitch - 1500) * 0.3f + sdk_manager.sdk_velocity_x;
	exp_velocity[1] = -(Remote.roll - 1500) * 0.3f + sdk_manager.sdk_velocity_y;
	if(ABS(exp_velocity[0])<5)
	{
		exp_velocity[0] = 0;
	}
	if(ABS(exp_velocity[1])<5)
	{
		exp_velocity[1] = 0;
	}
