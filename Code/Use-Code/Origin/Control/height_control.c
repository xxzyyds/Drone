...
void ALT_Ctrl(float dT_s)
{
	...
	if(exp_vel_transition[0]>0)
	{
		...
		exp_vel_transition[1] = exp_vel_transition[0] *MAX_EXP_WZ_VEL_UP + sdk_manager.sdk_alt_out;// + PROGRAM_CTRL_VELOCITY_Z + PROGRAM_CTRL_AUTO_TAKEOFF_LAND_VEL;
	}
	else
	{
		exp_vel_transition[1] = exp_vel_transition[0] *MAX_EXP_WZ_VEL_DW + sdk_manager.sdk_alt_out;// + PROGRAM_CTRL_VELOCITY_Z + PROGRAM_CTRL_AUTO_TAKEOFF_LAND_VEL;
	}
	//
