...
void sdk_init()
{
    ...
    sdk_manager.sdk_yaw_d_angle = 0.2f;

    sdk_manager.sdk_auto_takeoff = true;//自动起飞
...
{
    //更新位置的时候记得旋转坐标系
    sdk_manager.location_x += ANO_OF.DX2 * cosf(PIDGroup_desired_yaw_pos_tmp / 57.3f);
    sdk_manager.location_y += ANO_OF.DY2 * cosf(PIDGroup_desired_yaw_pos_tmp / 57.3f);
...
void sdk_round_set(float distance, float angle, uint8_t R_)
{
    ...
    */
	...
	// t = angle / 0.2(角速度) * 100（Hz）时间延长100倍
		// perimeter divide 
